  ...
  const TreeCard = ({ tree, isOwner }: { tree: Tree; isOwner: boolean }) => (
    <div className={styles.card}>
      ...
      <div className={styles.cardActions}>
        ...
        <button className={styles.btnSecondary} onClick={() => copyLink(tree.shareSlug)}>
          🔗 Copy link
        </button>
        <button className={styles.btnSecondary} onClick={() => router.push(`/tree/${tree.shareSlug}`)}>
          👁 Preview
        </button>
        <button className={styles.btnSecondary} onClick={() => setSettingsTree(tree)}>
          ⚙️ Settings
        </button>
        {isOwner && (
          <button className={styles.btnDanger} onClick={() => handleDelete(tree)}>Delete</button>
        )}
      </div>
    </div>
  )
